...
contract Users {
    ...
    function getUserInfo(address _address) external view returns (string memory name, string memory email) {
        require(
            msg.sender == _address ||
        ...
        name = userInfoMap[_address].name;
        email = userInfoMap[_address].email;
        return (name, email);
    }
